...
def get_response(chat_history, user_text, model):
...
# Once the name is entered, proceed with the chatbot
    ...
    if "chat_history_en" not in st.session_state:
        ...
        st.session_state.chat_history_en = [
            {"role": "system", "content": st.session_state.primer},
            {"role": "assistant", "content": starter_message.text}
        ]
                    ...
                    with st.chat_message("assistant", avatar=st.session_state.avatars['assistant']):
                        with st.spinner(THINKING):
                            ...
                            response, st.session_state.chat_history_en = get_response(
                                user_text=translated_input.text,
                                chat_history=st.session_state.chat_history_en,
                                model=args.model_id,
                            )
                            translated_response = translator.translate_text(response, target_lang="DE")
                            st.session_state.chat_history.append({'role': 'assistant', 'content': translated_response.text})
                            st.markdown(translated_response.text)
                else:
                    with st.chat_message("assistant", avatar=st.session_state.avatars['assistant']):
                        with st.spinner(THINKING):
                            ...
                            translated_input = translator.translate_text(st.session_state.user_text,
                                                                         target_lang="EN-US")
                            response, st.session_state.chat_history_en = get_response(
                                user_text=f"Give answer to: {translated_input.text}. End the conversation and say goodbye.",
                                chat_history=st.session_state.chat_history_en,
                                model=args.model_id,
                            )
                            translated_response = translator.translate_text(response, target_lang="DE")
